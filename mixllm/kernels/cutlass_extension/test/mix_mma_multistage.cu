...
#include "test/unit/gemm/threadblock/mma_multistage_testbed.h"
...
int main(int argc, const char ** argv) {
  ...
  for(; options.problem_size.m() <= 512; options.problem_size.m() *= 2){
    ...
    assert(cutlass::reference::host::TensorNorm(matrix_C_computed.host_view()) > 0);
    ...
    std::cout << "stage=" << NumStages \
              ...
              << ", TILE_M="  << TILESIZE_M \
              << ", TILE_N="  << TILESIZE_N;
    std::cout << ", r = " << options.ratio;
    std::cout << ", M = " << options.problem_size.m() << ", N = " << options.problem_size.n() << ", K = " << options.problem_size.k();

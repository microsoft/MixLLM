...
if __name__=='__main__':
    ...
    f = open('vllm-test.txt','a')
    print(f'////////////////////////////',file = f)
    print(f' {datetime.datetime.now()} vllm performace test',file = f)
    print(f'{device},{torch_version}',file = f)
    engine_path = '/workspace/models/Mistral-7B-Instruct-v0.3/'
    prompt_num = 800
    share_degree_list = [2,4,8,16]
    context_len = 2000
    prompt_len = 200
    vllm_share = False
    enable_chunked_prefill = False
    max_num_batched_tokens = 2048 if enable_chunked_prefill else 32768
    ...
    print(f'engine_path:{engine_path},enable_chunked:{enable_chunked_prefill},max_num_batched_tokens:{max_num_batched_tokens},vllm_share:{vllm_share}',file = f)
    print(f'prompt_num share_degree     time throughput',file = f)

    # run vllm baseline test
    vllm = LLM(model=engine_path,
               enable_chunked_prefill=enable_chunked_prefill,
               max_num_batched_tokens=max_num_batched_tokens,
               tensor_parallel_size=1,
               disable_sliding_window=True,
               block_size=64,
               enable_ahead_of_prefix_clustering = True
    ...
    for share_degree in share_degree_list:
        ...
        t1 = time.time()
        output = vllm.generate(prompt_token_ids=benchmark_input,
                               sampling_params=sampling_params
                               )
        t2 = time.time()
        print(f'{prompt_num:10} {(share_degree):12} {t2-t1:8.2f} {prompt_num/(t2-t1):10.2f}',file = f)

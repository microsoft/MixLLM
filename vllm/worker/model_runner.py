...
from vllm.attention.backends.utils import CommonAttentionState, ContextSharingMeta
...
from vllm.utils import (DeviceMemoryProfiler, GiB_bytes, PyObjectCache,
                        ...
                        weak_ref_tensor,make_tensor_with_pad)
...
class ModelInputForGPUBuilder(ModelRunnerInputBuilderBase[ModelInputForGPU]):
    ...
    class InterDataForSeqGroup:
        ...
        def __init__(
                ...
                *,
                ...
                csgroup_common_request_id: Optional[str] = None, 
                csgroup_common_length: int = 0, 
                csgroup_common_block_tables: Optional[List[int]] = None, 
        ):
            if reinit:
            ...
            self.csgroup_common_request_id = csgroup_common_request_id
            self.csgroup_common_length = csgroup_common_length
            self.csgroup_common_block_tables = csgroup_common_block_tables
    ...
    def _compute_for_batchllm(
        self, inter_data: InterDataForSeqGroup, seq_idx: int,
            seq_group_metadata: SequenceGroupMetadata
    ):
        """Fix seq_len/context_len/input_position when batch
        """
        if not self.batchllm_enabled:
            return 
        
        batchllm_this_group_hit = \
            inter_data.csgroup_common_request_id is not None \
            and inter_data.csgroup_common_request_id != inter_data.request_id
        ...
        if batchllm_this_group_hit:
            ...
            inter_data.input_positions[seq_idx] = inter_data.input_positions[seq_idx][:-(inter_data.seq_lens[seq_idx] - inter_data.context_lens[seq_idx])]
            inter_data.input_positions[seq_idx].extend(range(inter_data.context_lens[seq_idx], inter_data.seq_lens[seq_idx]))
    ...
    def add_seq_group(self, seq_group_metadata: SequenceGroupMetadata):
        ...
        inter_data = self.init_cached_inter_data(
            ...
            csgroup_common_request_id=seq_group_metadata.csgroup_common_request_id,
            csgroup_common_length = 0 if seq_group_metadata.csgroup_common_length is None 
                else seq_group_metadata.csgroup_common_length,
            csgroup_common_block_tables = seq_group_metadata.csgroup_common_block_tables,
        )
...
class GPUModelRunnerBase(ModelRunnerBase[TModelInputForGPU]):
    ...
    def __init__(
        ...
        self.graph_group_block_tables = np.zeros(
            (max(_BATCH_SIZES_TO_CAPTURE), self.get_max_block_per_batch()),
            dtype=np.int32)
        self.graph_request_block_tables = np.zeros(
            (max(_BATCH_SIZES_TO_CAPTURE), self.get_max_block_per_batch()),
            dtype=np.int32)
    ...
    def warmup_batchllm(self, kv_caches):
        from vllm.attention.backends.utils import warmup_batchllm
        return warmup_batchllm(self, kv_caches)

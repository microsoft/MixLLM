...
from vllm.core.csgroup_manager import CSGroupManager
...
class SchedulingBudget:
    ...
    token_budget: int
    max_num_seqs: int
    max_num_kv_usage: int
    token_batching_degree: int # only work for chunked prefill
    ...
    def can_schedule(self, *, num_new_tokens: int, num_new_seqs: int, enable_chunking: bool = False):
        assert num_new_tokens != 0
        assert num_new_seqs != 0
        max_num_seqs = self.max_num_seqs if not enable_chunking else self.max_num_seqs + self.token_batching_degree
        return (self.num_batched_tokens + num_new_tokens <= self.token_budget
                and self.num_curr_seqs + num_new_seqs <= max_num_seqs)

    def remaining_token_budget(self):
        return self.token_budget - self.num_batched_tokens

    def remaining_kv_usage(self):
        return self.max_num_kv_usage - self._num_kv_usage

    def is_kv_usage_exceeds(self):
        return self._num_kv_usage >= self.max_num_kv_usage
    ...
    def add_kv_usage(self, req_id: str, num_kv_usage: int):
        self._num_kv_usage += num_kv_usage
    ...
    @property
    def num_kv_usage(self):
        return self._num_kv_usage
...
class Scheduler:
    ...
    def __init__(
        ...
        self.max_kv_usage = 1000000000
        # NOTE(fangtaosong): max_kv_usage need to be set to a smaller value for token-batching work around
        ...
        self.csgroup_manager = CSGroupManager(self, self.block_manager)
        ...
        # Sequence groups in the WAITING state for CSGroup common input.
        # Contain new common prefill or preempted requests.
        self.dist_ready: Deque[SequenceGroup] = deque()
        # Sequence groups in the WAITING state for CSGroup dist input.
        # Contain new dist prefill or preempted requests.
        self.dist_waiting: Deque[SequenceGroup] = deque()
    ...
    def add_seq_group(self, seq_group: SequenceGroup, is_batchllm_prefix: Optional[bool] = None) -> None:
        # Add sequence groups to the waiting queue.
        if is_batchllm_prefix == True:
            self.waiting.append(seq_group)
            self.csgroup_manager.add_common_request(seq_group)
        elif is_batchllm_prefix == False:
            self.dist_waiting.append(seq_group)
            self.csgroup_manager.add_dist_request(seq_group)
            csgroup_common_request_id = self.csgroup_manager.get_common_request_id(seq_group)
            seq_group.update_csgroup_common_request_id(csgroup_common_request_id)
        else:
            self.waiting.append(seq_group)
    ...
    def has_unfinished_seqs(self) -> bool:
        return len(self.waiting) != 0 or len(self.running) != 0 or len(
            self.swapped) != 0 or len(self.dist_waiting) != 0
    ...
    def get_num_unfinished_seq_groups(self) -> int:
        return (len(self.waiting) + len(self.running) + len(self.swapped) +
                len(self.dist_waiting) - len(self.csgroup_manager.group_dict))
    ...
    def _schedule_running(
        ...
        while running_queue:
            if budget.is_kv_usage_exceeds():
                logger.warning(
                    "Blocked by KV cache restriction"
                )
                break
            ...
            kv_usage = self._get_kv_usage(seq_group, SequenceStatus.RUNNING,
                                          enable_chunking, budget)

            if num_running_tokens == 0 or budget.is_kv_usage_exceeds():
                # No budget => Stop
                break
            ...
            while not self._can_append_slots(seq_group, enable_chunking):
                ...
                budget.add_num_batched_tokens(seq_group.request_id,
                                              num_running_tokens)
                budget.add_kv_usage(seq_group.request_id, kv_usage)
    ...
    def _schedule_default(self) -> SchedulerOutputs:
        ...
        budget = SchedulingBudget(
            ...
            max_num_kv_usage=self.max_kv_usage,
            token_batching_degree=0
        )
    ...
    def _schedule_chunked_prefill(self) -> SchedulerOutputs:
        ...
        budget = SchedulingBudget(
            ...
            max_num_seqs=self.scheduler_config.max_num_seqs,
            max_num_kv_usage=self.max_kv_usage,
            token_batching_degree=self.scheduler_config.token_batching_degree,
        )
        ...
        assert (budget.num_batched_tokens <=
                self.scheduler_config.max_num_batched_tokens)
        assert budget.num_curr_seqs <= self.scheduler_config.max_num_seqs + self.scheduler_config.token_batching_degree
    ...
    def schedule(
        ...
        for i, scheduled_seq_group in enumerate(
            ...
            csgroup_common_request_id, csgroup_common_block_tables, csgroup_common_length = \
                self.csgroup_manager.get_common_request(seq_group)
            ...
            for seq in seq_group.get_seqs(status=SequenceStatus.RUNNING):
                ...
                # NOTE(batchllm): Since batchllm's kernels performs not as good as the vllm's
                # original kernels in some scenarios, here we keep the `full` block tables
                # of one request in a shared-prefix group for better utilizing vllm's kernels.
                block_tables[seq_id] = (
                    csgroup_common_block_tables + self.block_manager.get_block_table(seq)
                    if self.csgroup_manager.is_csgroup_dist(seq_group)
            ...
            if is_first_prefill or not self.scheduler_config.send_delta_data:
                seq_group_metadata = SequenceGroupMetadata(
                    ...
                    csgroup_common_request_id=csgroup_common_request_id,
                    csgroup_common_block_tables=csgroup_common_block_tables,
                    csgroup_common_length=csgroup_common_length,
                    is_prompt=is_prompt,
                    seq_data=seq_data,
    ...
    def _free_finished_seq_group(self, seq_group: SequenceGroup) -> None:
        if seq_group.is_finished():
            ...
            # group are processed.
            self.csgroup_manager.remove_finished_request(seq_group)

        # Free finished seqs, except the seq_group of "common" part.
        if not self.csgroup_manager.is_csgroup_common(seq_group):
    ...
    def free_finished_seq_groups(self) -> None:
        ...
        if self.dist_ready:
            self.waiting.extendleft(self.dist_ready)
            self.dist_ready = deque()
    ...
    def _get_kv_usage(self, seq_group: SequenceGroup,
                      status: SequenceStatus, enable_chunking: bool,
                      budget: SchedulingBudget) -> int:
        ...
        seqs = seq_group.get_seqs(status=status)
        for seq in seqs:
            num_kv_usage += seq.get_len()
        # Chunk if a running request cannot fit in.
        # If number of seq > 1, it means it is doing beam search in a
        # decode phase. Do not chunk in that case.
        if enable_chunking and len(seqs) == 1:
            pass
            num_kv_usage = min(num_kv_usage,
                               budget.remaining_kv_usage())
        return num_kv_usage

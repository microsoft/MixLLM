...
def _cached_get_attn_backend(
    ...
    elif backend == _Backend.BATCH_LLM:
        logger.info("Using BatchLLM backend.")
        from vllm.attention.backends.batch_llm import BatchLLMBackend
        return BatchLLMBackend
...
def which_attn_to_use(head_size: int,
    ...
    if backend_by_global_setting is not None:
        ...
        if envs.VLLM_USE_BATCHLLM:
            envs.VLLM_ATTENTION_BACKEND = "BATCH_LLM"
    ...
    if current_platform.is_rocm():
        ...
        elif selected_backend == _Backend.BATCH_LLM:
            selected_backend = _Backend.ROCM_BATCH_LLM
        else:
            logger.info("%s is not supported in AMD GPUs.", selected_backend)
            selected_backend = _Backend.ROCM_FLASH

        if selected_backend == _Backend.ROCM_FLASH or selected_backend == _Backend.ROCM_BATCH_LLM:

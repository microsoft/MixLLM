...
def prefix_cluster_preprocess(cluster, prompt_token_ids):
    """
    Check whether there is a structure like  ...,[[a,...],[[b,...],[c,d,...]]],..., a(prefix),b,c are not lists
    """
    def has_nested_structure(input_data):
        if not isinstance(input_data, list):
            return False

        has_single_list = False
        ...
        for item in input_data:
            if isinstance(item, list):
                if any(isinstance(x, list) for x in item):
                    ...
                    has_single_list = True


                if has_nested_structure(item):
                    return True

            if has_single_list and has_nested_list:
                return True

        return False

    if has_nested_structure(prompt_token_ids):
        logger.warning("The input prompt has a original nested structure, the prefix clustering is skipped.")
        return None, prompt_token_ids
    for ind, prompt in enumerate(prompt_token_ids):
        cluster.add_prompt(prompt, ind)
    cluster.finalize()
    map_list, input_tokens = cluster.get_clustered_prompts(return_map_list=True)
    return map_list, input_tokens

def prefix_cluster_postprocess(map_list, output_list):
    assert len(map_list) == len(output_list), "# output < # input, please check"
...
def print_batchllm_info(cluster):
    ...
    print(
        f"Avg. sharing degree: {avg_sharing_degree:.2f}, "
        ...
        f"saving ratio: {(logical_tokens - physical_tokens) / logical_tokens * 100:.2f}%"
    )
...
class PromptPrefixCluster:
    ...
    class TokensNode(object):
        ...
        def __init__(self,
            ...
            self.children = dict()  # {node_id: TokensNode}
    ...
    def add_prompt(self, input_ids: List[int], prompt_id):
        ...
        if len(input_ids) != 0:
            ...
            node = new_node
        leaf_node = self._create_node(None, node, prompt_id)
    ...
    def _enlarge_1st_prefix_tokens_deprecated(self, node: TokensNode,
        ...
        for fork in children:
            if fork.input_ids is None:  # Leaf node.
                continue
            ...
            if (node.parent is not None) and (gain > 0):
                ...
                if len(node.children) == 1:
                    # The node becomes a normal node. Merge the last child into
                    # it.
    ...
    def _enlarge_1st_prefix_tokens(self, node: TokensNode):
        ...
        for child in children:
            if child.input_ids is None:  # Leaf node.
                continue
            ...
            for gchild in grandchildren:
                if gchild.input_ids is None:  # Leaf node.
                    continue
    ...
    def summarize(self):
        ...
        for cluster in clustered_prompts:
            ...
            logical_tokens += prefix_tokens * group_size + cluster_distinct_tokens
    ...
    def _print_pretty(self, node: TokensNode, token_num: int, width: int = 4):
        ...
        node_token_num = len(
            node.input_ids) if node.input_ids is not None else 0

...
class ModelConfig:
    ...
    def __init__(
            ...
            enforce_fused_cs: Optional[bool]= False,) -> None:
        self.model = model
        ...
        self.enforce_fused_cs = enforce_fused_cs
...
class CacheConfig:
    ...
    def verify_with_batchllm_config(self, speculative_config) -> None:
        use_batchllm = envs.VLLM_USE_BATCHLLM
        if not use_batchllm:
            return

        if self.sliding_window is not None:
            raise NotImplementedError(
                "BatchLLM features is not supported with sliding window. "
                "Run with --disable-sliding-window to use BatchLLM.")
        if speculative_config:
            raise NotImplementedError(
                "BatchLLM features is not supported with speculative decoding. ")
...
class SchedulerConfig:
    ...
    def __init__(self,
        ...
        self.token_batching_degree = token_batching_degree
        self.policy = policy
        self.enable_ahead_of_prefix_clustering = enable_ahead_of_prefix_clustering
        self._verify_args()
...
class VllmConfig:
    ...
    def __post_init__(self):
        ...
        if self.cache_config is not None:
            ...
            self.cache_config.verify_with_batchllm_config(self.speculative_config)
